...
router.post('/getStocksCurrent', (req, res) => {
    connection.query('SELECT Stock_Name,Price As price, Company as company FROM stocks s WHERE NOT EXISTS ( SELECT * FROM stocks t WHERE t.Stock_Name = s.Stock_Name AND s.`Timestamp` > t.`Timestamp`)',
    function (error, results, fields) {
        if (error) throw error;

        console.log('getStocksCurrent result: ', results);
